...
\begin{itemize}
  ...
  \item \textbf{Efficiency.} Deployment should not require large-model inference. Mathematical denoising carries the main computational load; optional refinement uses a TinyResidualRefiner runK checkpoint (56\,129 parameters, $\approx$225\,KB). DeepFilterNet3 is confined to offline teacher generation during distillation training.
...
\begin{enumerate}
  ...
  \item \textbf{Optional distillation.} \texttt{distill/infer\_student\_refine.py} loads \texttt{student\_runK.pt} and writes refined outputs to \texttt{outputs/distill\_refined\_runK/}.
...
\begin{figure}[H]
  ...
  \resizebox{0.92\textwidth}{!}{%
    \input{figures/web_dataflow.tex}%
  }
...
Upon \texttt{POST /api/process}, the backend invokes \texttt{denoise\_runner.run\_denoise()} with the selected method (\texttt{auto}, explicit algorithm name, \texttt{noisereduce}, \texttt{wiener}, or \texttt{deepfilter}). If \texttt{run\_distill\_refine=true}, \texttt{distill\_refiner.refine\_with\_student()} loads \texttt{student\_runK.pt} via \texttt{resolve\_distill\_checkpoint()} and post-processes the denoised WAV. Users select \textbf{auto} plus checkbox \textbf{Run distill refine} in the upload UI (\chapref{ch:web}). Finally, \texttt{metrics\_service.build\_metrics\_and\_plots()} generates SNR statistics, residual descriptors, and 24+ grouped Plotly figures stored as JSON for the chart center.
