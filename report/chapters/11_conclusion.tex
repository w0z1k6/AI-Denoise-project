...
TinyResidualRefiner (2641 parameters, $\alpha=0.8$) improved routed RMS on \textbf{all 15 scenes}, lowering the mean from $0.00878$ to $0.00816$ and closing approximately 39\% of the mean gap to the DeepFilterNet3 teacher ($0.00595$) while adding only $\approx 8$\,ms refine latency on CPU. Gains were largest on chirp and babble ($\Delta e = 0.00191$ and $0.00143$), where math routing leaves structured residual error. No scene regressed relative to its routed WAV under the fixed \texttt{runD} checkpoint, validating optional refinement as a safe quality knob.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    Per-scene override routing & 0.00878 & 13 / 15 \\
    + TinyResidualRefiner (\texttt{runD}) & 0.00816 & 15 / 15 \\
    DeepFilterNet3 teacher (offline) & 0.00595 & 14 / 15 vs.\ refined \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  Experiment with multi-teacher ensembles (e.g., DeepFilterNet + metricGAN-style targets), perceptual losses (PESQ/STOI surrogates), and phase-aware masks. Priority-scene oversampling should continue to emphasize large Cohen's $d$ deficits and high baseline kurtosis.
