...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \If{$req.\mathit{run\_distill\_refine}$}
      \State $\hat{x} \gets \textsc{RefineWithStudent}(\hat{x}, \mathit{input}, \texttt{student\_runD.pt})$
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l X l}
    ...
    \texttt{auto} + \texttt{run\_distill\_refine} & Routed output + \texttt{student\_runD.pt} & Hybrid (\chapref{ch:dl}) \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item Run \texttt{auto} with \texttt{run\_distill\_refine=true} (hybrid);
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Train student         & \texttt{python distill/train\_student\_distill.py --tag runD} \\
    Infer distill refine  & \texttt{python distill/infer\_student\_refine.py --checkpoint ...} \\
    AB RMS evaluation     & \texttt{python distill/eval\_distill\_ab.py} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{deepFilter} CLI + \texttt{models/DeepFilterNet3/} --- \texttt{deepfilter} mode and teacher generation;
  \item \textbf{student\_runD.pt} --- required for \texttt{run\_distill\_refine}; silently skipped if missing.
\end{itemize}
