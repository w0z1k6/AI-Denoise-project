...
Scene analysis produces human-readable reports: band-wise SNR, local-SNR histograms, Cohen's $d$ separability, and baseline fixed-subtraction residual kurtosis (\chapref{ch:scene}). Routing emits structured logs (\texttt{route\_name}, \texttt{chain}, \texttt{reason}) so every benchmark WAV can be traced to a physics-matched module sequence (\chapref{ch:routing}). Mathematical stages expose STFT-domain gains, notch frequencies, NMF activations, and WPE coefficients---quantities an engineer can correlate with spectrograms (Figures~\ref{fig:spectrogram_comparison}--\ref{fig:spec_scene15}). The distillation refiner is small enough to audit (2641 parameters, single residual mask head) and is applied \emph{after} routed math, so failures can be attributed to routing, a specific front-end, or the residual corrector independently.

\paragraph{What is sacrificed in absolute error.}
On the 15-scene benchmark, mean RMS error progresses as $0.00936$ (monolithic OM-LSA) $\to$ $0.00878$ (override routing) $\to$ $0.00816$ (distill refined) versus $0.00595$ (teacher), Table~\ref{tab:distill_summary}. The student closes roughly 39\% of the remaining gap between routed math and teacher in mean RMS while adding only $\approx 8$\,ms inference (\chapref{ch:dl}, Table~\ref{tab:runtime}). Structured interference still dominates the residual: scene09 chirp refined RMS ($0.01495$) remains more than $3\times$ the teacher ($0.00447$) even after \texttt{chirp\_notch} and distillation. Thus the hybrid stack is best viewed as a \emph{practical middle tier}---better than a single classical enhancer on heterogeneous noise, but not a drop-in replacement for full DeepFilterNet deployment when absolute fidelity is the only objective.

\paragraph{Deployment economics.}
DeepFilterNet3 CLI processing takes $\approx 2.9$\,s per 5.6\,s clip on the project workstation; the full hybrid pipeline (route + refine) stays under $\approx 200$\,ms for typical override modules. Teacher generation for all 15 scenes required $\approx 44$\,s offline, whereas student training (\texttt{runD}) completed in 20.1\,s and never runs at user inference except as an optional refine pass. For a web demo or batch analysis tool, this cost asymmetry justifies distillation: the teacher informs training once; deployment carries only routed math plus an optional tiny network.
...
\begin{itemize}
  ...
  TinyResidualRefiner improves routed RMS on all 15 scenes (mean $\Delta e = 6.19\times 10^{-4}$) but cannot erase large teacher gaps on chirp, bursts, or babble where the teacher itself achieves $e_{\mathrm{RMS}} \approx 0.004$--$0.011$ while routed math remains above $0.012$--$0.017$. The student learns a \emph{residual mask} on STFT magnitudes conditioned on routed output; it does not replace physics-matched front-ends that remove sweeping ridges or impulsive events. Highly structured interference therefore remains the dominant failure mode.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{1.55cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{1.35cm} c >{\raggedright\arraybackslash}X@{}}
    ...
    \projectshort{} routed + refine & $0.00816$ & $\approx 100$--200\,ms & High & Heterogeneous benchmark; demo + research \\
    DeepFilterNet3 teacher & $0.00595$ & $\approx 2900$\,ms & Low & Offline quality reference; distillation teacher \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Priority scenes (02, 09, 10--12) exhibited the largest Cohen's $d$ deficits between speech and noise STFT statistics. That prioritization aligned with the largest distillation gains (scene09 $\Delta=0.00191$, scene12 $\Delta=0.00143$). Investing engineering effort in chirp ridge cancellation and babble WPE chains yielded more benchmark improvement than further tweaking OM-LSA gain floors on easy stationary scenes.
...
Training the student on teacher--routed STFT pairs worked uniformly (15/15 improvements) because the network corrects \emph{consistent} residual patterns left by math (slurred harmonics, ridge remnants, babble smear). It did not learn to invert gross routing errors on chirp when routed files were suboptimal. The design lesson: distillation budget is best spent on scenes where routed output is already plausible---matching the oversampling of priority scenes in \texttt{runD} training (\chapref{ch:dl}).
