...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \texttt{run\_distill\_refine} & bool, \texttt{false} & Apply \texttt{student\_runD.pt} after routing \\
...
python analysis/denoise_selection/distill/eval_distill_ab.py
...
\begin{table}[H]
  \centering
  \caption{\texttt{distill\_config.json} training defaults (\texttt{runD} checkpoint).}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    \texttt{max\_steps} / \texttt{max\_train\_seconds} & 1200 / 900 \\
    ...
    Loss weights (\texttt{distill\_l1}, \texttt{mag\_compress}, \texttt{clean\_aux\_mag}) & 1.0, 0.3, 0.15 \\
    Teacher mask clip & $[0.2,\, 1.8]$ \\
    Hard-scene weights (S02, S09, S11) & 3.0, 3.5, 1.5 \\
    Architecture & Conv2D $2\!\to\!16\!\to\!16\!\to\!1$, $3\times 3$ kernels \\
    Parameters & 2641 (\texttt{student\_runD.pt}) \\
    \texttt{seed} & 42 \\
    \bottomrule
  \end{tabular}
\end{table}
